...
function SineCurvedCharts({ chartData, handleItemClick }) {
  ...
  return (
    <Box w='100%' p='16px'>
      ...
      <Box display={'flex'} gap={'29px'}>
        {chartData?.sineCurve?.leftHind?.length > 0 && <SineCurve color={colors.lightMediumBlue} chartData={chartData} data={chartData?.sineCurve?.leftHind} type='hind' />}
        {chartData?.sineCurve?.rightHind?.length > 0 && <SineCurve color={colors.lightPurple} chartData={chartData} data={chartData?.sineCurve?.rightHind} type='hind' />}
        {chartData?.sineCurve?.straighthind?.length > 0 && <SineCurve color={colors.lightMustard} chartData={chartData} data={chartData?.sineCurve?.straighthind} type='hind' />}
      </Box>
    </Box>
  )
}
